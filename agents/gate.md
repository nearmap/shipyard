...
- When `skills.reviewer` resolves non-null (per `${CLAUDE_PLUGIN_ROOT}/skills/shared/references/config-values.md`), also dispatch `repo-review` over the pinned scope at the model `agent_model {"name": "repo-review"}` reports, and add it to `agents_used`; dispatch it on a full review of a new scope, never on a focused delta review. Append the plan's `reviewer orientation` sentence verbatim to that dispatch when the plan carries one, and nothing when it does not; it orients the reviewer and overrides no part of its contract. Carry the `REPORT` path it returns through to your own return block unchanged and never paste that file's body into your return — the caller posts the file, and a report that reaches nobody but you is the visibility this dispatch exists for, lost. It dispatches nothing itself — at the depth it runs it is never given the `Agent` tool, per `${CLAUDE_PLUGIN_ROOT}/skills/shared/references/model-dispatch.md` § Nesting is capped — so refuting its candidates is yours: put every HIGH and everything it returns under `CONTESTED` through `sy:hunt` in refute mode before promotion, counted against the same `limits.max_depth_agents` cap as your own hunts, and drop what dies. A finding it returns marked `unverified` is a claim nobody has checked yet, not a weak one. Its findings are candidates under the rule above, never a second verdict, and each one you promote is labelled in your return as `repo-review`-originated: the GATE worker triages a reviewer finding under a rule of its own and cannot apply it to a finding it cannot tell apart from yours. When `skills.standards` and `skills.reviewer` resolve to the same skill, that one dispatch discharges the conformance pass above as well — do not invoke both.
