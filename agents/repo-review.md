...
2. Resolve the output location as the repository-keyed scratch root that `${CLAUDE_PLUGIN_ROOT}/skills/shared/references/agent-returns.md` § Persisting a report names, never a task-keyed identifier. That root is the one the mutation guard sandboxes; a task-keyed path resolves to a sibling directory outside it, so every write the skill attempts there is denied and the run dies mid-review.
...
Findings that live only in your return reach nobody but `sy:gate`. Persist the same findings as a standalone human-readable report before returning, per `${CLAUDE_PLUGIN_ROOT}/skills/shared/references/agent-returns.md` § Persisting a report — `kind` `repo-review`, `scope` `<REVIEWED_SHA>` — with one deliberate deviation from that recipe: the name carries no timestamp, `repo-review-<REVIEWED_SHA>.md`, one file per review scope. A re-dispatch over the same scope overwrites it on purpose, because `${CLAUDE_PLUGIN_ROOT}/skills/ship/references/immutable-gate.md` § Posting the reviewer's report posts once per scope and checks for that SHA's existing comment; a second stamped file for the same SHA would be a report nothing ever posts. Name its absolute path in your return block; the caller posts it to the pull request. Never post it yourself: the pull request is the caller's surface, and `/sy:pr` owns every write to it — one place composes, posts and reconciles comments, so they cannot drift into two conventions.
