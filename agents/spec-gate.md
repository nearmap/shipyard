...
- Check the three completeness fields mechanically, then honestly: present-and-`none` on work that plainly touches docs or a rendered visual is a finding; a `none` pre-gate checkpoint is never one — that call is the plan author's alone (`skills/spec/SKILL.md`'s own completeness note).
