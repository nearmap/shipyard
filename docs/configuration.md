...
| `scratch.dir` | no | Root for per-identifier ephemeral working directories (verbose command output, throwaway artefacts, `/sy:ship` state). Resolve one identifier's with `scratch-dir <identifier>`, or the repository's own with `scratch-dir --repo`. Unset derives a directory under `~/.claude/shipyard/`. Absolute paths only, and never inside a checkout it resolves for — this backs `review_guard.py`'s hunt-mode write sandbox, so a relative or checkout-containing value is refused rather than resolved. |
