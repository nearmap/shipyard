...
| `SY_FRONTIER_FALLBACK` | both | no | `opus` | The model a phase falls back to once if its requested model cannot run (spend cap, rate limit, or a refusal), clamped up to that phase's own floor. If the reviewer's fallback also fails, ship returns `blocked` rather than promoting an unreviewed commit. |
