...
`/sy:ship` builds the approved plan and takes it to a reviewable PR. It branches from fresh `origin/main` into its own worktree, created under the sibling `<repo>-worktrees/` directory by default or under `worktree.root` when set (first checking that `main` has not drifted from the commit the plan assumed), implements the plan in order, gets CI green, and has the independent `sy:gate` agent review the exact pushed commits in an isolated, read-only checkout. When the plan declared a pre-gate checkpoint, ship pauses right after opening the draft PR to ask you whether this looks like the right thing — a direction check on the channel the plan named, never a correctness review — before `sy:gate` ever runs; a run whose plan declared none is unaffected. Fixes go back into the build worktree, and every new commit starts a new review scope. When the PR head, the CI-green commit, and the reviewed commit are the same commit, ship posts the evidence to the ticket, moves it to `in-review`, and stops.
