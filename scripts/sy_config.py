...
def _logical_repo(start: Path) -> Path:
    ...
    raise SystemExit(
        f"sy_config: {str(start)!r}'s repository has no resolvable working tree: its shared git "
        f"directory is {common}, and {common.parent} is not itself a checkout of it. This is most "
        "often a submodule whose own identity git cannot currently resolve — for example after "
        "`git submodule deinit` without a later `git submodule update --init`, or a nested or "
        "detached submodule layout git records no `core.worktree` for. Run `git submodule update "
        "--init` for the affected submodule and retry; resolving anyway risks two unrelated "
        "repositories sharing one scratch directory."
    )
