...
def _logical_repo(start: Path) -> Path:
    ...
    if common.parent.name == "modules" and common.parent.parent.name == ".git":
        raise SystemExit(
            f"sy_config: {str(start)!r} is inside a git submodule whose own working tree cannot be "
            f"resolved (no 'core.worktree' in {common}) — most likely `git submodule deinit` ran "
            "without a later `git submodule update --init` for it. Run `git submodule update --init` "
            "for this submodule and retry; resolving anyway would key its scratch/worktree identifier "
            "on the fixed string 'modules', shared by every other submodule on the machine in the "
            "same state."
        )
