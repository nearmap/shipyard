...
2. **Simplicity.** Is this the smallest change that delivers the goal? Look for a config toggle where an unconditional behaviour would do, a new abstraction with one caller, ordered steps that collapse into one, and scope the goal does not require. The plan's own prose is in scope, on three objective triggers, all of them governed by `${CLAUDE_PLUGIN_ROOT}/skills/shared/references/context-economy.md`. Two apply to the whole plan: prose restating a decision argued elsewhere in the same plan, and prose that narrates what a cited anchor already shows. Neither is a length budget — a long plan whose every paragraph is load-bearing passes this axis. The third applies to the `/sy:ship` half alone, where the only reader is an implementer: rationale a builder cannot act on. It is not a budget either.
