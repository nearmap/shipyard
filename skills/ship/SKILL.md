...
- Model tier is a quality floor, and the parent wires it mechanically: START's and BUILD's dispatches each carry an explicit `Agent` invocation model override, never omitted, read from the plan's per-phase declaration and clamped up to that worker's own floor. The parent never infers a phase's model from a single word and never assumes a worker inherits the parent session's model, because a worker's own frontmatter model always wins over an omitted override. The profile's `START` and `BUILD` models are what those workers run at; the `GATE` worker's own dispatch from the parent stays at its fixed `sonnet` tier regardless of profile, and it is the GATE worker itself — not the parent — that reads the profile's `GATE` model and passes it on as the model override for its nested `sy:gate` call. Each phase worker runs at least its declared tier (BUILD stays at least opus) and the ship profile may raise but never lower it, so the parent passes no override that drops a worker below its own floor. Effort is the profile's cost lever: the parent applies the plan's effort to workers to match the work but never lowers review effort (`sy:gate` stays max).
