...
- Workers never prompt the user; each returns `done`, `needs-decision`, `needs-trace`, `bail-to-spec`, or `blocked` per the worker contract. The parent resolves `needs-decision` from plan/standards/code first and asks you via `AskUserQuestion` only when genuinely ambiguous, and resolves `needs-trace` by dispatching the trace itself without asking you; either way it then dispatches a fresh continuation worker from the checkpoint.
