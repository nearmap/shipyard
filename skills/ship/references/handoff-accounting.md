...
- `gate_rounds_budget_base` — the `gate_rounds_total` a raise-budget disposition was taken at, or `0` for the run that never hit the cap. Never `null`: it is the other half of the escalation ceiling, since `gate_rounds_total - gate_rounds_budget_base` — not `gate_rounds_total` itself — is what `ship.escalation.max_gate_rounds` is measured against (`${CLAUDE_PLUGIN_ROOT}/skills/ship/references/immutable-gate.md` § Fix cycle). A non-zero value is the signal worth reading: this run asked for more rounds and got them, and `gate_rounds_total` says how many it spent in all.
