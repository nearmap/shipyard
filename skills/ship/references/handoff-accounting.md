...
- `plan_divergence_count` — `len(accepted_deviations) + count(plan_supersede_events)`, computed mechanically from the state file at handoff, never hand-incremented as the run goes. A deviation that was *considered and declined* is not a divergence; it belongs in `deviations_declined`.
...
- `pregate_checkpoint_changes_requested` — how many times the checkpoint sent work back to BUILD before an eventual proceed. `0` either way — for a run that declared no checkpoint and for one that was waved straight through. Unlike `plan_divergence_count`, this one is genuinely incremented by the parent as the run goes (`${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` § Pre-gate checkpoint).
