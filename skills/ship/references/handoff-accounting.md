...
- `gate_rounds_total` — **every** GATE pass over the current head, including the passes that found nothing and the ones whose findings were all rejected. That is the difference from `review_fix_rounds`, which counts only the subset folded into a following commit: a run that ground through six clean-verdict-chasing rounds and changed nothing records six here and zero there. This is the count the escalation ceiling (`ship.escalation.max_gate_rounds`) is measured against, so under-counting it hides the runs that never converged.
