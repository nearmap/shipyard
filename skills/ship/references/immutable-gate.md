...
**Narrow-fix reviewer cadence.** Not every fix round needs a fresh automated-reviewer request. When a round's entire diff is confined to resolving thread(s) the automated reviewer already raised — no lines outside what those threads flagged, no new file, no behavior beyond the fix itself — `sy:gate`'s own focused delta review (still frontier model, max effort, the reviewer of record for correctness) is sufficient for that round: skip the fresh request, but still reply to and resolve the originating thread(s) through `/sy:pr` as usual. The moment a round's diff touches anything beyond what the automated reviewer has already seen and flagged, request it again — and request it at least once more before promotion if it has not yet reviewed the actual promoted head. This changes request cadence only, never coverage: the completion bar's requirement that the automated reviewer's threads are reconciled, and that its last review differs from the promoted head by nothing but already-covered fixes, is unchanged.
