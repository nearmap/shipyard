...
Every doc the plan names in `docs requiring updates` is a verification obligation on the same footing: before the draft PR, either update it in this branch or read it and confirm on inspection it is already accurate, and record which of the two for each named doc. The field is not a hint for incidental doc-sync — a named doc left neither updated nor inspected is an undischarged obligation, and an undischargeable one returns to `/sy:spec` like any other.
