...
Track build progress as a slice manifest in `phase_checkpoint` (per slice: `pending|committed|integrated`, with SHAs), written through `ship_state_update` after each integration and so durable before the return, so a `needs-decision` or `needs-trace` return — or a return that never reaches the parent at all — resumes at the next pending slice and re-does no integrated work. A slice's `source` is `plan` by default — its content is the cited plan step — or `pregate_revision`, whose content is the slice's own `spec` field instead (see below); nothing else about a slice's shape changes.

At the very start of any continuation, before resuming at the next pending slice, check the state brief's `pregate_checkpoint_request_text`. When it is set, the `/sy:ship` parent is relaying a pre-gate-checkpoint "request changes" reply. Fold it in **unless** your manifest already carries a `pregate_revision` entry whose `spec` is that exact text — that entry is this same reply already folded in, and the manifest being durable before the return is why a set field no longer means "not folded in yet" by construction (`${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` § Pre-gate checkpoint). When that dedup match fires the drop is never silent: this continuation's return must report in its `CHECKPOINT` field that the relayed request is already folded in, naming the matching `pregate_revision` entry by `id` and status, so a user restating a request can see which entry answered it. Folding it in is one new entry appended to your own manifest with `source: pregate_revision`, `status: pending`, and `spec` set to that text verbatim; its `id` and SHAs follow however you already key a slice. From there it is an ordinary slice: run it through the delegated protocol above using `spec` in place of a plan-step citation, and your own judgment (recorded as an `accepted_deviation`) for anchors and acceptance criteria, since a checkpoint reply names neither. `phase_checkpoint` stays entirely worker-authored: the parent never writes a slice itself, only the `pregate_checkpoint_request_text` field it hands off. The new pending entry is part of whatever `phase_checkpoint` this continuation eventually returns — `done`, `needs-decision`, `needs-trace`, `bail-to-spec`, or `blocked` alike — never something added only on a clean `done`.

After integration, run acceptance tests and standards-required formatter/linter/type checks; route verbose runs (full suite, linters, type checks) through logs in the task's resolved scratch directory — `scratch_dir($TASK_KEY)` throughout this reference means the `path` the `scratch_dir` tool reports for `{"identifier": "$TASK_KEY"}` — and read back only failures and summary lines. Discharge every verification obligation with its named evidence; an undischargeable obligation returns to `/sy:spec`. Where acceptance criteria describe observable behaviour, execute the behaviour (a runner kept in that same directory is fine) and capture the output as acceptance evidence — tests alone discharge only test-shaped criteria.
