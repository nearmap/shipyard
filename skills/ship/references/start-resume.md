...
This phase runs as the `sy:ship-start` worker and returns the state brief per the worker contract.
...
4. Resolve standards in the `repo-standards` delegate (running `/sy:standards resolve <task scope>` at the model `agent_model {"name": "repo-standards"}` reports, added to `agents_used`) that returns only the retained contract — authority, implementation contract, primitives, risk lenses; rule-file reads stay out of the ship context.
...
Each phase's `*_model_requested` is written when that phase is dispatched and its `*_model_observed` only once the usage transcript confirms what ran, so the `build_model_*` and `review_model_*` pairs stay `null` until the parent dispatches those phases. `pregate_checkpoint_channel` is stamped once here too, from the plan's `pre-gate checkpoint` field in the normalized form the block above shows, alongside `pregate_checkpoint_gate_dispatched` and `pregate_checkpoint_request_text` at their initial values; `${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` § Pre-gate checkpoint owns how all five are later set and re-checked.

The state file is local resume state, not shared truth. Never prune unrelated worktrees or paths. Every write to it after this seed goes through `ship_state_update`, which merges the fields named and leaves the rest of the file alone — a hand-edit rewrites the whole file and drops whatever a differently-versioned build wrote into it. `phase_checkpoint` is the active worker's idempotent resume anchor (e.g. a slice manifest with per-slice status), passed to any continuation worker. `phase_active` is the parent's own, never a worker's, and `gate_rounds_total`/`gate_rounds_budget_base`/`gate_round_log` are GATE's live fix-cycle round accounting; `${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` § Worker contract and `references/immutable-gate.md` § Fix cycle own all three. All of them are stamped once here so a resume reads them rather than inferring them, and an older state file carrying none of them reads as `null`/`0`/`0`/`[]`. `gate_round_log` is worker-authored and appended to, one entry per round — round number, `reviewed_sha`, verdict summary, and per finding, accepted or rejected, a stable `root_cause_key` and its disposition — never replaced wholesale the way `phase_checkpoint` is, because it is what carries recurrence across a dispatch boundary rather than within one worker. Write it as a real list of mappings; a literal block scalar does not survive the round trip a merge write makes. `plan_path`, `plan_comment_id` and `plan_version` are stamped here for the same reason, from what the parent's own `plan_file` call reported, and read as `null` on a state file written before they existed. A `null` pin is not a resume failure and needs no migration: the comparison is skipped rather than failed (`${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` § State router).

Return `done` with the state brief.
