...
2. Sibling interfaces and blockers off the parent Epic are stated in this worker's dispatch prompt by the parent, the same precedent as `START_MODEL` in `## Resolve start model` below: neither this worker nor `sy:sweep` holds a tracker read, so an instruction to go read the Epic here would be dead.
