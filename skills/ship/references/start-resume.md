...
6. Load `ship-state.yaml` from the task's resolved scratch directory — `scratch_dir {"identifier": "$TASK_KEY"}`, whose reported `path` is that directory — if present. Any `memory_refutations` still listed in the loaded state are pending, not history: the parent applies each via `memory_refute` immediately on resume, before dispatching anything, and clears the list from state once applied — the same drain rule as an in-flight worker return (`${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` § Worker contract). A crash between a worker's return and the parent's write is exactly what that durable mirror exists for, and an undrained list means the refuted anchor is still being read back as if it held.
