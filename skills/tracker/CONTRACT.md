...
A comment, once posted, is never edited. An execution plan is versioned instead: the plan on a `task`/`bug` is the comment carrying the highest `# Execution Plan vN` heading, each version names the one it supersedes, and every older version stays posted and readable in full as history. Never use a "latest-looking comment wins" heuristic — the version number is the selector, not position or recency. This is a core convention; the adapter only provides `post-comment`/`get-issue`. For `/sy:ship` it is not left to the caller to uphold: the `plan_file` tool is what enforces it, refusing when there is no plan comment at all, refusing by comment id rather than picking when two comments claim the same highest version, and refusing when a `vN (continued)` stub is stranded at or above the version it would otherwise select — a stub numbered above it means a later version's second half is posted without its first, and selecting the older complete plan there would hand on a plan the issue has already moved past.

Retraction goes the same way for a plan posted in error or prematurely, because immutability removes the only other route: it cannot be demoted or emptied out, so it is retracted by posting the intended content as the next version, which says on the record what was wrong with the last one. A version left standing is not an endorsement of it; only the highest one is ever read.

A plan posted against the **wrong work item** is the one case that route does not reach: there is no intended content for that issue to post, and a version can never mean "no plan" — a "void" or "retracted" highest version is still the highest version, still selected, still materialised, and nothing reads such a marker. Close it on the issue itself instead, through the issue's own status or a label, and leave the plan comment standing as history.
