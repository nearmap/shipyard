...
A zero-result scan is evidence, not proof of safety, and a declared-opaque artifact carries no *scrub* result at all rather than a zero standing in for a pass that never ran, while its scan result is as real as on any other upload. Neither pass substitutes for the other: the scrub catches known values verbatim regardless of shape; the scanner catches shapes it recognises regardless of whether this process ever held the value. Findings name a rule, not a value — inspect the file, redact contextual secrets the rules miss (organisation-specific identifiers, private signed URLs, `.env` values), and re-attach. If safe redaction is uncertain, stop rather than publishing.
