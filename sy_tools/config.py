...
def _all_worktrees(common: Path, logical: Path) -> list[Path]:
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        ...
        if not pointed.is_absolute():
            raise ConfigError(
                f"{str(gitdir_file)!r} names {str(pointed)!r}, which did not resolve to an absolute "
                "path, so this worktree's own location cannot be determined and so cannot be guarded."
            )
