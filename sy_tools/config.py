...
def repo_scratch_dir(start: Path | None = None) -> Path:
    ...
    if _same_directory(directory, logical) or any(
        ...
        raise ConfigError(
            f"the resolved scratch directory {directory.resolve()} contains this repository's own "
            ...
            "supposed to keep hunt out of it; check for a misconfigured scratch.dir in a committed or "
            "local .shipyard/config.json."
        )
    return directory
